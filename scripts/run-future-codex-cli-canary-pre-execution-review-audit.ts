...
const REQUIRED_PACKAGE_SCRIPTS = {
  ...
  "audit:future-codex-cli-canary-execution-gate":
    "tsx scripts/run-future-codex-cli-canary-execution-gate-audit.ts",
  "audit:future-codex-cli-canary-authorization-packet":
    "tsx scripts/run-future-codex-cli-canary-authorization-packet-audit.ts",
...
const FORBIDDEN_REVIEW_AUTHORIZATION_MARKERS = [
  "run the canary now",
  ...
  "target file: `*`",
  "This review provides a future execution command"
] as const;

export interface FutureCodexCliCanaryPreExecutionReviewAuditInput {
...
export interface FutureCodexCliCanaryPreExecutionReviewAuditResult {
  ...
  checks: {
    ...
    preExecutionReviewRecorded: boolean;
    reviewNonExecuting: boolean;
    priorExecutionGateRecorded: boolean;
    priorAuthorizationPacketRecorded: boolean;
    priorChecklistRecorded: boolean;
    priorControlledGateRecorded: boolean;
    exactReviewFieldsRecorded: boolean;
...
export type FutureCodexCliCanaryPreExecutionReviewAuditOutputFormat =
  | "text"
  | "json";

export async function collectFutureCodexCliCanaryPreExecutionReviewAuditInput(
  cwd = process.cwd()
  ...
  return {
    ...
    reviewDocText: await read(cwd, REVIEW_DOC),
...
export function reviewFutureCodexCliCanaryPreExecutionReviewAudit(
  input: FutureCodexCliCanaryPreExecutionReviewAuditInput
): FutureCodexCliCanaryPreExecutionReviewAuditResult {
  ...
  const checks = {
    ...
    preExecutionReviewRecorded:
      input.reviewDocText.includes(
        "FUTURE_CODEX_CLI_CANARY_PRE_EXECUTION_REVIEW_RECORDED"
      ),
    reviewNonExecuting: reviewIsNonExecuting(input.reviewDocText),
    priorExecutionGateRecorded:
    ...
    exactReviewFieldsRecorded: REQUIRED_REVIEW_FIELDS.every((field) =>
      input.reviewDocText.includes(field)
    ),
    ...
    countersStayZero: countersStayZero(input.reviewDocText),
...
export function formatFutureCodexCliCanaryPreExecutionReviewAuditResult(
  review: FutureCodexCliCanaryPreExecutionReviewAuditResult,
  format: FutureCodexCliCanaryPreExecutionReviewAuditOutputFormat = "text"
): string {
  if (format === "json") {
  ...
  return [
    "Future Codex CLI canary pre-execution review audit",
...
function reviewIsNonExecuting(text: string): boolean {
  ...
  return normalized.includes("does not authorize, run, or simulate provider execute")
    ...
    && normalized.includes("This review does not provide or run a future execution command")
    && normalized.includes("It is not the canary execution itself")
    && FORBIDDEN_REVIEW_AUTHORIZATION_MARKERS.every((marker) =>
...
function countersStayZero(text: string): boolean {
  return text.includes("provider execute calls during pre-execution review")
    && text.includes("real Codex CLI calls during pre-execution review")
    && text.includes("workspace-write execute calls during pre-execution review")
    && text.includes("canary file writes during pre-execution review");
}

function evidenceSanitized(input: FutureCodexCliCanaryPreExecutionReviewAuditInput): boolean {
  return [
...
function collectReasons(checks: Record<string, boolean>): string[] {
  return Object.entries(checks)
    .filter(([, ok]) => !ok)
    .map(([name]) => `future_codex_cli_canary_pre_execution_review_${name}`);
}
...
async function main(): Promise<void> {
  const input = await collectFutureCodexCliCanaryPreExecutionReviewAuditInput();
  const review = reviewFutureCodexCliCanaryPreExecutionReviewAudit(input);
  const format = process.argv.includes("--json") ? "json" : "text";

  console.log(formatFutureCodexCliCanaryPreExecutionReviewAuditResult(review, format));
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    console.error(
      "Future Codex CLI canary pre-execution review audit failed:",
