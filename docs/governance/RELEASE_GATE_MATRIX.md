...
| Merge integrity | GitHub `pull_request_target` `Merge Integrity` check, executed from the exact base SHA | Detects active PR-body merge locks and validates an allowed approver's exact repository, PR, head, time, and merge scope without executing PR code in the privileged event context. | Merge whenever a lock is active without a valid structured authorization. |
