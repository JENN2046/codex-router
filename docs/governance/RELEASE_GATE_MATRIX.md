...
| Merge integrity | GitHub `pull_request` `Merge Integrity` check | Detects active PR-body merge locks and validates an allowed approver's exact repository, PR, head, time, and merge scope. | Merge whenever a lock is active without a valid structured authorization. |
