...
| Agent task control dispatch sandbox dry-run | active / taskbook-only | No | Phase 18 defines the future sandbox-only task-control contract witness boundary. It is planning-only, must use a separate sandbox task-control adapter kind, and does not invoke an adapter, Codex CLI, provider, sub-agent runtime, shell/process execution, workspace-write, external write, or production recovery execution. |
